...
- 渲染：任务在应用内启动 Remotion（`npx --no-install remotion render`），进度日志通过事件流实时显示、可取消（按任务）、错误原因与输出位置（应用数据目录 `renders/<jobId>/vertical-draft.mp4`）回传界面；**批量渲染队列**：一键渲染全部已审核草稿，支持并发 1-3 与暂停/恢复（批间暂停、恢复续跑）、失败计数、完成汇总；完成后自动（≥2 条成功）或手动生成**预览合辑**（ffmpeg concat 流复制拼接全部成片为单条 mp4）；窄屏顶栏搜索收纳为跳转按钮
