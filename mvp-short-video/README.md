...
- 三栏工具界面：侧边栏 / 草稿列表 / 检查器；Apple HIG 桌面工具质感（设计系统 v2：修复语义色变量缺失、按钮体系主操作蓝/次级中性、徽章收敛为低对比 pill、顶栏 52px 紧凑、草稿行精简为 2 个徽章、检查器头部单状态徽章）；深色模式三态（自动跟随系统 / 强制浅色 / 强制深色，侧边栏底部一键切换并持久化，Apple 深色色板 + 波形 canvas 颜色随主题）；窄屏保留侧边栏与列表，检查器折叠为可关闭抽屉（容器查询：抽屉窄时预览/编辑自动单列）；键盘操作（⌘S 保存版本、⌘⏎ 审核、Esc 关闭、/ 聚焦搜索）、focus-visible、ARIA、破坏性操作确认
...
- 差异对比与审校：新增"差异对比"视图——草稿 A/B 起步、可动态添加任意草稿列（+添加草稿列/移除）、字段级分镜对比（画面大字/辅助文案/时长/素材，变化高亮并标注变化类型）、发布文案对比、时长/素材/审核状态摘要、"下一对"快速轮换、逐字段把任意一列合并回草稿 A、直接对 A 标记审核通过；审核清单每行可一键"对比"
...
- 渲染：任务在应用内启动 Remotion（`npx --no-install remotion render`），进度日志通过事件流实时显示、可取消（按任务）、错误原因与输出位置（应用数据目录 `renders/<jobId>/vertical-draft.mp4`）回传界面；**批量渲染队列**：一键渲染全部已审核草稿，支持并发 1-3、批间暂停/恢复、失败自动重试一次、失败计数与完成汇总；完成后自动（≥2 条成功）或手动生成**预览合辑**（ffmpeg concat，可配 canvas 生成的片头标题卡 2s + 统一 h264/aac 压制或流复制）；窄屏顶栏搜索收纳为跳转按钮
- 生产打包：`npm run tauri:build` 会先把渲染运行时（裁剪后的 node_modules + 源码 + **预生成的 Remotion bundle**，约 240MB）打进 .app，桌面端**开箱即用、首次渲染零等待**（bundle 只读即可渲染）；渲染优先复用已有 headless shell（开发模式）或系统 Chrome（自动加 `--allow-insecure-localhost`），避免生产环境下载 Headless Shell 卡住
- **字幕轨样式控制**：检查器内直接调整字幕轨字号（36/44/54/64）与颜色（白/黄/绿/蓝 4 预设），或一键移除字幕轨；素材库可"导出全部字幕包"（全部已转写素材的 SRT 合集，含双语时用双语 SRT）
...
- **字幕烧录与 SRT**：已转写素材可"嵌入字幕轨"（原文或双语）进当前草稿——`subtitleTrack` 随导出 JSON 快照，Remotion `VerticalDraft` 底部安全区内按时间烧录（双语两行、字号/颜色可调、克制不抢画面）；素材可直接导出单语/双语 SRT（按索引配对，桌面端保存对话框）
...
1. 深色模式下 Remotion 渲染的 fallback 背景适配（视频内暗色主题）。
2. 渲染队列失败重试次数可配置与任务优先级。
3. 合辑转场（xfade）与自动片尾落版。
