...
- 生成质量：确定性 seed、相似度指纹去重（10 条草稿在钩子/痛点/证据/CTA 上差异明显）、行业模板（民宿/餐饮/农产品/探店/培训/本地服务）、文案长度约束（标题 ≤34、画面大字 ≤32、正文 ≤200）
...
- **转写**：ffmpeg 提取 16k 单声道音频 → whisper.cpp 转写（模型如 `ggml-base.bin`）→ 生成 SRT 与分段字幕，写入素材元数据 `transcript`，素材卡片可预览前几行
...
- **字幕时间轴编辑器**：素材库可展开"对齐字幕"面板——时间轴可视化（分镜轨道 + 字幕段色块，归属分镜颜色实时反映）、逐段指派归属分镜（自动/不填入/指定分镜）、按指派生成并填入草稿，未指派段落自动按窗口归属，实现人工可控的文案对齐；字幕段支持点击选中（Shift 多选）、直接拖拽（整组拖入目标分镜）、键盘指派（Enter/空格）；指派结果可保存到素材元数据跨会话保留，也可随填入草稿一并保存
