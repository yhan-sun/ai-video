...
- 渲染：任务在应用内启动 Remotion（`npx --no-install remotion render`），进度日志通过事件流实时显示、可取消、错误原因与输出位置（应用数据目录 `renders/<jobId>/vertical-draft.mp4`）回传界面；**批量渲染队列**：一键串行渲染全部已审核草稿（静默模式逐个执行、失败计数、完成汇总），窄屏顶栏搜索收纳为跳转按钮（跳转全部草稿并聚焦搜索框）
