...
- 生产打包：`npm run tauri:build` 会先把渲染运行时（裁剪后的 node_modules + 源码，约 214MB，已剔除 source map 与 prettier 语言插件）打进 .app，桌面端**开箱即用**——首次渲染自动生成 Remotion bundle 缓存到应用数据目录（资源目录只读，缓存写可写位置）；渲染优先复用已有 headless shell（开发模式）或系统 Chrome（自动加 `--allow-insecure-localhost`），避免生产环境下载 Headless Shell 卡住
...
1. 字幕轨样式控制 UI（字号/颜色/位置在检查器可视化调整）。
2. 生产打包的运行时体积优化（bundle 提前生成进 .app，跳过首次渲染等待）。
3. 草稿批量导出为多语言字幕包。
